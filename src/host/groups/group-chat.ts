...
export function buildGroupTurnPrompt(args: { member: GroupMember; group: GroupDescription; peers: readonly GroupMember[]; newMessages: readonly GroupMessage[]; addressed?: boolean }): string { const close = args.addressed === true ? `It's your turn, ${args.member.name}. The user @mentioned you. Reply in character with a single SendMessage — your final answer only, no progress notes or receipts — do not pass.` : `It's your turn, ${args.member.name}. Reply in character with a single SendMessage if you have something worth adding, or send \"(pass)\" if you don't. One message: the final answer only — no progress notes, no receipts.`; const lines = [formatGroupChatTag(args.group, args.peers), args.newMessages.length === 0 ? "No new messages in the room since your last turn." : `New messages in the room (oldest first):\n${formatGroupHistory(args.newMessages, args.member.id)}`, "", close]; return lines.join("\n"); }
