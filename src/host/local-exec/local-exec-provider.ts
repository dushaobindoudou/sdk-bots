...
export interface LocalExecFileStore {
...
export interface SandLocalExecProviderOptions {
  ...
  readonly root?: string; readonly terminalsFolder?: string; readonly computerId?: string; readonly computerLabel?: string; readonly maxFileBytes?: number; readonly variant?: string;
  readonly fetch?: typeof fetch;
  readonly fileStore?: LocalExecFileStore;
  readonly postResponses?: (frames: readonly LocalExecResponseFrame[]) => Promise<boolean>;
...
export class SandLocalExecProvider {
  ...
  constructor(private readonly options: SandLocalExecProviderOptions) {
    this.root = options.root ?? resolveLocalExecRoot(); this.terminalsFolder = options.terminalsFolder ?? join(this.root, "terminals");
  ...
  announce(): void { void this.currentSupervised().then((supervised) => this.enqueue({ kind: "hello", localRoot: this.root, terminalsFolder: this.terminalsFolder, computerId: this.computerId, label: this.computerLabel, ...(supervised === undefined ? {} : { supervised }), ...(this.options.variant === undefined ? {} : { variant: this.options.variant }) })); }
  ...
  private async postBatch(frames: readonly LocalExecResponseFrame[], deadline = this.dataPostDeadline): Promise<boolean> {
    if (this.options.postResponses != null) {
      try { return await this.options.postResponses(frames); }
      catch (error) { console.error(`[local-exec-provider] dropping ${frames.length}-frame in-process response batch: ${errorLogTag(error)}`); return false; }
    }
  ...
  private async handleUpload(requestId: string, path: string, bytesBase64: string): Promise<void> { try { const approximate = Math.floor(bytesBase64.length * 3 / 4); if (approximate > this.maxFileBytes) { this.enqueue({ kind: "file-error", requestId, error: localExecFileTooLargeMessage(approximate, this.maxFileBytes) }); return; } const bytes = Buffer.from(bytesBase64, "base64"); if (this.options.fileStore != null) await this.options.fileStore.write(path, bytes); else { const target = await containPath({ root: this.root, path }); await writeFileAtomic(target, bytes); } this.enqueue({ kind: "file", requestId }); } catch (error) { this.enqueue({ kind: "file-error", requestId, error: errorMessage(error) }); } }
  private async handleDownload(requestId: string, path: string): Promise<void> { try { let data: Uint8Array; if (this.options.fileStore != null) { data = await this.options.fileStore.read(path); if (data.byteLength > this.maxFileBytes) { this.enqueue({ kind: "file-error", requestId, error: localExecFileTooLargeMessage(data.byteLength, this.maxFileBytes) }); return; } } else { const target = await containPath({ root: this.root, path }); const size = await regularFileSizeBytes(target); if (size !== undefined && size > this.maxFileBytes) { this.enqueue({ kind: "file-error", requestId, error: localExecFileTooLargeMessage(size, this.maxFileBytes) }); return; } data = await readFile(target); } this.enqueue({ kind: "file", requestId, bytesBase64: Buffer.from(data).toString("base64") }); } catch (error) { this.enqueue({ kind: "file-error", requestId, error: errorMessage(error) }); } }
}
