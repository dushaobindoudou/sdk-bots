...
import { createBoxBackedLocalExecExecutor, createBoxLocalExecFileStore } from "./box-backed-executor.js";
import {
...
export const NO_LOCAL_EXEC_CONNECTION_MESSAGE = "local-exec daemon has no gateway connection yet (set SAND_LOCAL_EXEC_GATEWAY_URL or wait for the host to write the connection file)";
export const LOCAL_EXEC_CONNECTION_PATH = "/sand-box/local-exec-connection";

export function localExecConnectionFromEnv(env: NodeJS.ProcessEnv = process.env): LocalExecConnection | null {
  const baseUrl = env.SAND_LOCAL_EXEC_GATEWAY_URL?.trim() ?? "";
  if (baseUrl.length === 0) return null;
  const token = env.SAND_GATEWAY_TOKEN?.trim();
  return { baseUrl, ...(token == null || token.length === 0 ? {} : { token }) };
}
...
export async function runLocalExecDaemon(options: RunLocalExecDaemonOptions = {}): Promise<{ close(): Promise<void> }> {
  ...
  const executor = options.executor ?? createBoxBackedLocalExecExecutor(resolveBoxExecEndpoint());
  ...
  const providerOptions: SandLocalExecProviderOptions = {
    ...
    root: boxComputerWorkspaceRoot(),
    terminalsFolder: BOX_TERMINALS_FOLDER,
    computerId: process.env.SAND_BOX_COMPUTER_ID?.trim() || "box",
    computerLabel: process.env.SAND_BOX_COMPUTER_LABEL?.trim() || "box",
    resolveConnection: async () => { const connection = await readLocalExecDaemonConnection(connectionPath) ?? localExecConnectionFromEnv(); if (connection == null) throw new SandLocalExecConnectionError(NO_LOCAL_EXEC_CONNECTION_MESSAGE); return connection; },
