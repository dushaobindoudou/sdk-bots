...
import { readExecutorResource, type ReadExecutor } from "../../lib/agent-exec/read.js";
...
export interface FileTransferAccessor { get(resource: typeof shellExecutorResource): ShellExecutor; get(resource: typeof writeExecutorResource): WriteExecutor; get(resource: typeof readExecutorResource): ReadExecutor }
...
export async function readFileBytesViaExecDaemon(ctx: Context, accessor: FileTransferAccessor, boxPath: string): Promise<Uint8Array> {
  const result = (await accessor.get(readExecutorResource).execute(ctx, new ReadArgs({ path: boxPath, encodingHint: "latin1" }))).result;
  if (result.case === "success") {
    const output = result.value.output;
    if (output.case === "data") return output.value;
    if (output.case === "content") return Buffer.from(output.value, "latin1");
    throw new SandBoxFileTransferError(`download from box ${boxPath} returned ${output.case ?? "empty"}`);
  }
  if (result.case === "fileNotFound") throw new SandBoxFileTransferError(`download from box ${boxPath} failed (file missing)`);
  const reason = result.case === "error" ? result.value.error : result.case === "rejected" ? result.value.reason : result.case === "invalidFile" ? result.value.reason : result.case;
  throw new SandBoxFileTransferError(`download from box ${boxPath} failed (${result.case}): ${reason}`);
}
