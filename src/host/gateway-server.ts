...
import { createGatewayLogTap, type GatewayLogTap } from "./gateway-log-tap.js";
...
/** Timing-safe compare for credentials arriving outside headers (EventSource cannot send Authorization). */
...
export interface GatewayServerDeps {
  ...
  /** Console log ring backing GET /logs and GET /logs/stream; created (and disposed) by startGatewayServer when omitted. */
  readonly logs?: GatewayLogTap;
}
...
export async function handleRequest(deps: GatewayServerDeps, req: IncomingMessage, res: ServerResponse): Promise<void> {
  ...
  const logsBacklog = req.method === "GET" && url.pathname === "/logs"; const logsStream = req.method === "GET" && url.pathname === "/logs/stream";
  if (!(events || prepare || avatar || localRequests || localResponses || webRequests || webResponses || command || logsBacklog || logsStream)) return respondError(res, 404, `not found: ${req.method} ${url.pathname}`);
  ...
  // SSE/avatar GETs may carry the gateway token as ?token= because EventSource and <img> cannot set headers.
  const headerlessGet = events || avatar || logsBacklog || logsStream;
  if (deps.authToken != null && !isAuthorized(req, deps.authToken) && !(headerlessGet && tokenEquals(url.searchParams.get("token"), deps.authToken))) return respondError(res, 401, "unauthorized");
  ...
  if (logsBacklog) { const rawLimit = url.searchParams.get("limit"); const limit = rawLimit == null ? 200 : Math.max(1, Math.min(Number.parseInt(rawLimit, 10) || 200, 2_000)); return respondJson(res, { entries: deps.logs?.recent(limit) ?? [] }); }
  if (logsStream) { if (deps.logs == null) return respondError(res, 404, "log stream not enabled"); return openSseStream(req, res, (write) => deps.logs!.subscribe((entry) => write(JSON.stringify({ channel: "logs", payload: entry })))); }
  return routeCommand(deps, url.pathname.slice(GATEWAY_API_PREFIX.length + 1), await readBody(req), res, req);
}

export async function startGatewayServer(deps: GatewayServerDeps) { const host = deps.host ?? "127.0.0.1"; const ownsLogs = deps.logs == null; const logs = deps.logs ?? createGatewayLogTap(); const listener = (req: IncomingMessage, res: ServerResponse) => { void handleRequest({ ...deps, logs }, req, res).catch((error) => { if (!res.headersSent) respondError(res, statusForCommandError(error), errorMessage(error)); else res.end(); }); }; const server = deps.tls == null ? createHttpServer(listener) : createHttpsServer({ cert: deps.tls.cert, key: deps.tls.key }, listener); await new Promise<void>((resolve, reject) => { server.once("error", reject); server.listen(deps.port ?? 0, host, () => { server.off("error", reject); resolve(); }); }); const address = server.address(); if (address == null || typeof address === "string") throw new Error("gateway did not bind a TCP address"); return { port: address.port, close: () => new Promise<void>((resolve, reject) => { server.closeAllConnections(); server.close((error) => error != null ? reject(error) : resolve()); }) }; }
